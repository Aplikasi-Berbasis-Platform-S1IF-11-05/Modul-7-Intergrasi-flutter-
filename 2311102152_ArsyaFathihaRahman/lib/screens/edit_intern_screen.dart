...
class _EditInternScreenState extends State<EditInternScreen> {
  ...
  late final TextEditingController _sekolahController;
  ...
  late final TextEditingController _tanggalSelesaiController;

  late String _statusMagang;
  bool _isLoading = false;

  @override
  void initState() {
    super.initState();
    _namaController = TextEditingController(text: widget.intern.namaLengkap);
    _emailController = TextEditingController(text: widget.intern.email);
    _teleponController = TextEditingController(text: widget.intern.nomorTelepon);
    _sekolahController =
        TextEditingController(text: widget.intern.asalSekolahKampus);
    _jurusanController = TextEditingController(text: widget.intern.jurusan);
    _posisiController = TextEditingController(text: widget.intern.posisiMagang);
    _alamatController = TextEditingController(text: widget.intern.alamat);
    _tanggalMulaiController =
        TextEditingController(text: widget.intern.tanggalMulai);
    _tanggalSelesaiController =
        TextEditingController(text: widget.intern.tanggalSelesai);
    _statusMagang = widget.intern.statusMagang;
  }
  ...
  Future<void> _updateIntern() async {
    ...
    try {
      final updatedIntern = widget.intern.copyWith(
      ...
      await _firestoreService.updateIntern(widget.intern.id!, updatedIntern);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Data peserta berhasil diperbarui!'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Peserta Magang'),
      ),
